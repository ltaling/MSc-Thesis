...
public :
   ...
   genie_analysis(std::string, std::string, int, int, int, int, double, double, double, int, int, double, double, double, double, int, int, TTree *tree=0);
...
// used to generate this class and read the Tree.
   if (tree == 0) {
    ...
    if((a_protMom_lb + a_protMom_ub) > 0 && a_protMom_lb < a_protMom_ub) {
        t_ProtMom_lb->SetVal(a_protMom_lb); t_ProtMom_ub->SetVal(a_protMom_ub);
