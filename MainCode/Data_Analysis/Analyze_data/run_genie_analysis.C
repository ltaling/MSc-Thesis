...
int main(int argc, char **argv)
{
  ...
  if( argc < 15 ){
    ...
    std::cout << "./genie_analysis target beam_energy 0/1 #rot 0/1/2 0/1 thetaEl_lb thetaEl_ub elMom_lb 0/1/2 0/1 thetaProt_lb thetaProt_ub protMom_lb protMom_ub" << std::endl;
    exit(1);
  }
  ...
  if (protMom_lb < 0) {
    std::cout << "Not a valid number for the proton momentum lower bound (parameter 14). It should be greater than or equal to 0. The given value is " << protMom_lb << std::endl;
    return 0;
  }
  if (protMom_ub < 0) {
    std::cout << "Not a valid number for the proton momentum upper bound (parameter 15). It should be greater than or equal to 0. The given value is " << protMom_ub << std::endl;
    return 0;
  }
  if (protMom_lb > protMom_ub) {
    std::cout << "The proton momentum lower bound (parameter 14) must be less than or equal to the proton momentum upper bound (parameter 15)." << std::endl;
    return 0;
  }
