// This macro draws the 1 dimensional histogram of the invariant mass, W.
...
void h1_pion_w (std::string file_name, bool pimin, bool pi0, bool piplu){
  ...
  if (pimin){
    ...
    if (h1_stacked[2] == nullptr){
      types = types + "#pi^{-}";
      h1_stacked[0] = h1_pi_w_0[0];
      h1_stacked[1] = h1_pi_w_0[1];
      h1_stacked[2] = h1_pi_w_0[2];
      h1_stacked[3] = h1_pi_w_0[3];
      h1_stacked[4] = h1_pi_w_0[4];
      h1_stacked[5] = h1_pi_w_0[5];
      }
    else{
      types = types + " & #pi^{-} ";
      h1_stacked[0]->Add(h1_pi_w_0[0]);
      h1_stacked[1]->Add(h1_pi_w_0[1]);
      h1_stacked[2]->Add(h1_pi_w_0[2]);
      h1_stacked[3]->Add(h1_pi_w_0[3]);
      h1_stacked[4]->Add(h1_pi_w_0[4]);
      h1_stacked[5]->Add(h1_pi_w_0[5]);
    }
  }
  if (pi0){
    ...
    if (h1_stacked[2] == nullptr){
      types = types + "#pi^{0}";
      h1_stacked[0] = h1_pi_w_1[0];
      h1_stacked[1] = h1_pi_w_1[1];
      h1_stacked[2] = h1_pi_w_1[2];
      h1_stacked[3] = h1_pi_w_1[3];
      h1_stacked[4] = h1_pi_w_1[4];
      h1_stacked[5] = h1_pi_w_1[5];
      }
    else{
      types = types + " & #pi^{0}";
      h1_stacked[0]->Add(h1_pi_w_1[0]);
      h1_stacked[1]->Add(h1_pi_w_1[1]);
      h1_stacked[2]->Add(h1_pi_w_1[2]);
      h1_stacked[3]->Add(h1_pi_w_1[3]);
      h1_stacked[4]->Add(h1_pi_w_1[4]);
      h1_stacked[5]->Add(h1_pi_w_1[5]);
    }
  }
  if (piplu){
    ...
    if (h1_stacked[2] == nullptr){
      types = types + "#pi^{+}";
      h1_stacked[0] = h1_pi_w_2[0];
      h1_stacked[1] = h1_pi_w_2[1];
      h1_stacked[2] = h1_pi_w_2[2];
      h1_stacked[3] = h1_pi_w_2[3];
      h1_stacked[4] = h1_pi_w_2[4];
      h1_stacked[5] = h1_pi_w_2[5];
      }
    else{
      types = types + " & #pi^{+}";
      h1_stacked[0]->Add(h1_pi_w_2[0]);
      h1_stacked[1]->Add(h1_pi_w_2[1]);
      h1_stacked[2]->Add(h1_pi_w_2[2]);
      h1_stacked[3]->Add(h1_pi_w_2[3]);
      h1_stacked[4]->Add(h1_pi_w_2[4]);
      h1_stacked[5]->Add(h1_pi_w_2[5]);
    }
  }
  ...
  //       h1_el_phientum[i]->Scale( 1./h1_el_momentum[i]->Integral(),"width"); // normalize the histogram to 1
      ...
      h1_toPlot[i]->GetXaxis()->SetTitle("W (GeV)");
  ...
  //        h1_el_momentum[i]->GetYaxis()->SetTitleSize( 0.06);
        ...
        h1_toPlot[i]->SetTitle("W Distribution of (1e"+TString(numProt)+"p1pi) Events "+TString(accept) + " for "+TString(types));
  ...
  double line_pos;
  if (pimin){line_pos = 1.445;}
  else if (piplu){line_pos = 1.405;}
  // Draw a line at the threshold
  TLine* line = new TLine(line_pos, 0, line_pos, 1.05*hist_max);
  ...
  //          it to the screen
  int num_DIS_events_below = h1_toPlot[0]->Integral(0,h1_toPlot[0]->FindBin(line_pos)-1);
  int num_1232_events_below = h1_toPlot[1]->Integral(0,h1_toPlot[1]->FindBin(line_pos)-1);
  int num_Other_events_below = h1_toPlot[2]->Integral(0,h1_toPlot[2]->FindBin(line_pos)-1);
  int num_Total_events_below = h1_toPlot[3]->Integral(0,h1_toPlot[3]->FindBin(line_pos)-1);

  int num_DIS_events_above = h1_toPlot[0]->Integral(h1_toPlot[0]->FindBin(line_pos),TMath::Ceil(maximumx));
  int num_1232_events_above = h1_toPlot[1]->Integral(h1_toPlot[1]->FindBin(line_pos),TMath::Ceil(maximumx));
  int num_Other_events_above = h1_toPlot[2]->Integral(h1_toPlot[2]->FindBin(line_pos),TMath::Ceil(maximumx));
  int num_Total_events_above = h1_toPlot[3]->Integral(h1_toPlot[3]->FindBin(line_pos),TMath::Ceil(maximumx));

  // create and draw a legend for our histograms
  TLegend *legend_above = new TLegend( 0.2, 0.5125, 0.5, 0.70);
  legend_above->SetHeader("Cut Events", "C");
  legend_above->AddEntry(line,Form("Cut at %3.3lf GeV",line_pos),"l");
  legend_above->AddEntry( h1_toPlot[0],Form("DIS (%d)",num_DIS_events_above),"l");
  legend_above->AddEntry( h1_toPlot[1],Form("#Delta(1232) (%d)",num_1232_events_above),"l");
  legend_above->AddEntry( h1_toPlot[2],Form("Other (%d)",num_Other_events_above),"l");
  legend_above->AddEntry( h1_toPlot[3],Form("Total (%d)",num_Total_events_above),"l");
  legend_above->SetBorderSize( 0);
  legend_above->SetFillStyle( 0);
  legend_above->Draw();

  // create and draw a legend for our histograms
  TLegend *legend_below = new TLegend( 0.2, 0.72, 0.5, 0.88);
  legend_below->SetHeader("Kept Events", "C");
  legend_below->AddEntry( h1_toPlot[0],Form("DIS (%d)",num_DIS_events_below),"l");
  legend_below->AddEntry( h1_toPlot[1],Form("#Delta(1232) (%d)",num_1232_events_below),"l");
  legend_below->AddEntry( h1_toPlot[2],Form("Other (%d)",num_Other_events_below),"l");
  legend_below->AddEntry( h1_toPlot[3],Form("Total (%d)",num_Total_events_below),"l");
  legend_below->SetBorderSize( 0);
  legend_below->SetFillStyle( 0);
  legend_below->Draw();

  // save this histogram as a PDF file
  std::string save_file_path ("invalid");
  save_file_path = "../output/h1_pion_w/"+TString(energ)+TString(nucleus)+TString(cuts)+TString(particles)+TString(numProt)+"P"+TString(a)+".pdf";
